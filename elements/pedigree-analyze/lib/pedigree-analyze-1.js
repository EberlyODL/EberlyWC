 ...
 */
class PedigreeAnalyze1 extends LitElement {
  /**
   * Store the tag name to make it easier to obtain directly.
   * @notice function name must be here for tooling to operate correctly
   */
  tag() {
    return "pedigree-analyze-1";
  }
  ...
  firstUpdated() {
    import(this.gsapCDN).then(() => {
      ...
      // SVG
      var lineone = svg.getElementById("lineone");
      var steptwo = svg.getElementById("steptwo");

      // Set Animate
      tl.set([lineone, steptwo], {
        alpha: 0
      });
...
customElements.define("pedigree-analyze-1", PedigreeAnalyze1);
export { PedigreeAnalyze1 };
